...
class CalendarFocus extends React.Component {
  ...
  addTime(hours: number, startEnd: "start" | "end") {
    const { drillDown, start, end } = this.props;
    if (startEnd == "start")
      drillDown(moment.utc(start).add(hours, "hours"), moment.utc(end));
    else drillDown(moment.utc(start), moment.utc(end).add(hours, "hours"));
  }
